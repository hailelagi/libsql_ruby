...
typedef const struct libsql_database *libsql_database_t;

typedef const struct libsql_connection *libsql_connection_t;

typedef const struct libsql_stmt *libsql_stmt_t;

typedef const struct libsql_rows *libsql_rows_t;

typedef const struct libsql_rows_future *libsql_rows_future_t;

typedef const struct libsql_row *libsql_row_t;
...
int libsql_open_remote(const char *url,
                       const char *auth_token,
                       libsql_database_t *out_db,
                       const char **out_err_msg);
...
int libsql_prepare(libsql_connection_t conn,
                   const char *sql,
                   libsql_stmt_t *out_stmt,
                   const char **out_err_msg);
...
int libsql_bind_blob(libsql_stmt_t stmt,
                     int idx,
...
int libsql_query(libsql_connection_t conn,
                 const char *sql,
...
int libsql_column_type(libsql_rows_t res,
                       libsql_row_t row,
                       int col,
                       int *out_type,
                       const char **out_err_msg);
